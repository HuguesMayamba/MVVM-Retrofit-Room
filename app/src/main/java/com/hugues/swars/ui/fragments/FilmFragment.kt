...
import com.hugues.swars.adapters.FilmAdapter
...
class FilmFragment : Fragment(R.layout.fragment_film) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        filmAdapter.setOnItemClickListener {
            ...
            findNavController().navigate(
                R.id.action_filmFragment_to_detailFilmFragment,
                bundle
            )
        }

        viewModel.film.observe(viewLifecycleOwner, Observer { response ->
            when(response) {
                is Resource.Success -> {
                    hideProgressBar()
                    hideErrorMessage()
                    response.data?.let { newsResponse ->
                        filmAdapter.differ.submitList(newsResponse.results.toList())
                        if(isLastPage) {
    ...
    private fun setupRecyclerView() {
        ...
        rvFlm.apply {
            ...
            addOnScrollListener(this@FilmFragment.scrollListener)
